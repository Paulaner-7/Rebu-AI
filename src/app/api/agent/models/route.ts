...
export async function GET(req: Request) {
  ...
  const db = writableDb();
  const saved = ((await db.prepare("SELECT value FROM settings WHERE key='modello_default'").get()) as { value: string } | undefined)?.value ?? DEFAULT_MODEL;
...
export async function POST(req: Request) {
  ...
  await writableDb().prepare("INSERT INTO settings (key,value) VALUES ('modello_default',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value").run(body.model);
