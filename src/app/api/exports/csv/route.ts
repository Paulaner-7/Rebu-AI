...
export async function GET(req: Request) {
  ...
  if (s.stato !== "CONCLUSA") {
    const n = ((await db.prepare("SELECT COUNT(*) AS n FROM purchases WHERE session_id=?").get(sid)) as { n: number }).n;
  ...
  try {
    const input = await buildExportInput(db, sid, u.searchParams.get("league") ?? "Rebu AI");
    const csv = generateLegheFantacalcioCsv(input, await knownIds(db, s.d));
