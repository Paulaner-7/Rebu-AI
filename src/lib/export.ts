...
export async function buildExportInput(db: Db, sid: number, leagueName: string): Promise<ExportInput> {
  const s = await db.prepare("SELECT dataset_version AS d, stato FROM auction_sessions WHERE id=?").get(sid) as { d: string; stato: string };
...
export async function knownIds(db: Db, dataset: string): Promise<Set<number>> {
  const rows = await db.prepare("SELECT official_id AS o FROM players WHERE dataset_version=?").all(dataset) as { o: number }[];
