...
describe("contratto agente", () => {
  it("system prompt: vincoli presenti", async () => {
    for (const s of ["Non inventi MAI numeri", "decisione finale è sua", "italiano"]) {
      expect(SYSTEM_PROMPT).toMatch(s);
    }
  });
  it("valida COMPRA ok, rifiuta >3 motivazioni e azioni ignote", async () => {
  ...
  it("estrae JSON da risposta con testo", async () => {
...
describe("tool statistici e KB", () => {
  it("tool statistiche e KB registrati", async () => {
  ...
  it("KB: ricerca per argomento e blocco esatto", async () => {
    expect(kbBlocco("KB-RIL-01")?.testo).toContain("tetto");
    const r = await kbCerca("esche far spendere avversari", 3);
    expect(r.risultati.length).toBeGreaterThan(0);
    expect(await kbDigest(900).length).toBeLessThanOrEqual(900);
  });

  it("execTool instrada i tre nuovi tool senza crash", async () => {
    ...
    await db.prepare("INSERT INTO players (dataset_version, official_id, nome, nome_norm, squadra, ruolo_classic) VALUES ('t',1,'Prova X','prova x','Roma','A')").run();
    const kb = await execTool(db, 0, "t", "consultaStrategia", { argomento: "KB-RIL-01" }) as { totale: number };
    expect(kb.totale).toBe(1);
    const st = await statsGiocatore(db, "t", 1) as { sintesi: { stagioni_coperte: number; gol_totali: number }; stagioni: unknown[] };
    expect(st.stagioni).toEqual([]);
    expect(st.sintesi.stagioni_coperte).toBe(0);
    expect(st.sintesi.gol_totali).toBe(0);
    const cl = await classificaStats(db, "xg", "", "", 5) as { top: unknown[] };
    expect(cl.top).toEqual([]);
    const bad = await classificaStats(db, "drop table", "", "", 5) as { errore: string };
