...
async function dbF() {
  ...
  const ins = await db.prepare(`INSERT INTO players (dataset_version, official_id, nome, nome_norm, squadra, ruolo_classic, ruolo_mantra, qt_a, fvm, is_titolare, qt_2526)
    VALUES ('t',?,?,?,?,?,?,?, ?,0,?)`);
  await ins.run(1, "Top A", "top a", "Roma", "A", "Pc", 38, 450, 40);   // rif 225
  await ins.run(2, "Mid C", "mid c", "Como", "C", "X", 20, null, 22);
  await ins.run(3, "Low D", "low d", "Lecce", "D", "Dc", 5, null, null); // rif media reparto
  await ins.run(4, "Top D", "top d", "Inter", "D", "E", 31, 240, 30);    // mod boost
  for (let i = 5; i <= 40; i++) await ins.run(i, `Riemp ${i}`, `riemp ${i}`, "Napoli", i % 2 ? "C" : "D", "X", 5, null, null);
  return db;
}

describe("motore prezzi", () => {
  it("riferimento: FVM/2, poi storiche, poi media reparto", async () => {
    const db = await dbF();
    const a = await prezzoRiferimento(db, "t", 1);
    expect(a.valore).toBe(225); expect(a.formula).toMatch("FVM / 2");
    const b = await prezzoRiferimento(db, "t", 2);
    expect(b.valore).toBe(21);
    const c = await prezzoRiferimento(db, "t", 3);
    expect(c.formula).toMatch("media");
  });
  it("tetto rosa vuota 476; dopo acquisto scala", async () => {
    const db = await dbF();
    const sid = await setupLeague(db, OTTO); await startAuction(db, sid, 0);
    expect(await (await tettoRilancio(db, sid, 1, 1)).tetto).toBe(476);
    await nominate(db, sid, 1); await sell(db, sid, { officialId: 1, managerId: 1, prezzo: 100, idem: "k" });
    expect(await (await tettoRilancio(db, sid, 1, 2)).tetto).toBe(500 - 100 - 23);
  });
  it("inflazione: neutra 1.0, poi 2.0 dopo strapagamento", async () => {
    const db = await dbF();
    const sid = await setupLeague(db, OTTO); await startAuction(db, sid, 0);
    expect(await (await inflazioneAsta(db, sid)).reparti.A.valore).toBe(1);
    await nominate(db, sid, 1); await sell(db, sid, { officialId: 1, managerId: 1, prezzo: 450, idem: "k" }); // rif 225
    expect(await (await inflazioneAsta(db, sid)).reparti.A.valore).toBe(2);
    const tc = await tettoConsigliato(db, sid, 2, 1);
    expect(tc.consigliato).toBe(Math.min(450, tc.tettoMax));
  });
  it("ranking: buco rosa prima, bonus modificatore su top D", async () => {
    const db = await dbF();
    const sid = await setupLeague(db, OTTO); await startAuction(db, sid, 0);
    const r = await prossimeChiamate(db, sid, 1, 3);
  ...
  it("matrice: 8 righe, conti tornano", async () => {
    const db = await dbF();
    const sid = await setupLeague(db, OTTO); await startAuction(db, sid, 0);
    await nominate(db, sid, 1); await sell(db, sid, { officialId: 1, managerId: 1, prezzo: 100, idem: "k" });
    const m = await matriceLega(db, sid);
